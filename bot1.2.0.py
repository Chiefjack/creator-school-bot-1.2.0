...
async def staffdecline(ctx, user: discord.Member):
	...
	message_to_declined_helper = (f"Unfortunately you weren't accepted into the Staff team. Feel free to reapply again, and f you would like a reason, please DM {ctx.message.author.mention}.")
...
async def staffhelp(ctx):
	...
	embed.add_field(name="**STAFF COMMANDS**", value= "**cs!supportresponse** - Responds to support requests. See pinned message in #support-log\n"
													...
													"**cs!warn** - Warns the tagged user. Usage: cs!warn 1 @ChiefJack_YT#4450 Stop spamming. Change cs!warn 1 to cs!warn 2 or cs!warn 3 depending on if they already have a previous warning.\n"
													"---")
	embed.set_thumbnail(url=bot.user.avatar_url)
	await bot.say(embed=embed)
...
async def warn1(ctx, user: discord.Member):
	...
	warning_message = (f"{user.mention} has been given Warning 1. This is your 1st Warning so there is no punishment. Please see {rules.mention} to reread the rules and see how Warnings work. The reason has been DMed to you if you have DMs enabled.\n**WARNED BY:** *{ctx.message.author}*")
...
async def warn2(ctx, user: discord.Member):
	...
	message = ctx.message.content
	messagereplace = message.replace("cs!warn 2", "")
	await bot.send_message(user, messagereplace)
	...
	emote_s = bot.get_channel("436083527281410058")
	feedback = bot.get_channel("460101527915462656")
	warning_message = (f"{user.mention} has been given Warning 2. This is your 2nd Warning; you have been muted for 10 minutes AND you no longer have access to Teacher and Staff application channels, {giveaways.mention}, {polls.mention}, {s_and_s.mention}, {emote_s.mention} and {feedback.mention}. Please see {rules.mention} to reread the rules and see how Warnings work. The reason has been DMed to you if you have DMs enabled.\n**WARNED BY:** *{ctx.message.author}*")
	warning_channel = bot.get_channel("414302090412949505")
	await bot.send_message(warning_channel, warning_message)
...
async def warn3(ctx, user: discord.Member):	
	message = ctx.message.content
	messagereplace = message.replace("cs!warn 3", "")
	await bot.send_message(user, messagereplace)
